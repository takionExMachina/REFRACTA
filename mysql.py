...
def queryPerson(rut):
    ...
    if rut is None:
        ...
        conn = pymysql.connect('localhost','root','@lF4bR@v0#123','REFRACTA')

        cursor = conn.cursor()

        cursor.execute('SELECT persona.rut,primer_nombre,segundo_nombre,apellido_paterno,apellido_materno,'
                       ...
                       'INNER JOIN region ON provincia.id_region = region.id_region WHERE persona.rut = \'%s\' '
                       'AND DATE(historial.fecha_caducidad) > DATE(NOW()) GROUP BY persona.rut' % rut)
